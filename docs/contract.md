...
| `agentRunSeconds`                             | non-negative number                         | Time the agent was executing; with `--transcript` and no stated figure, each turn from its prompt to its last agent record, waits capped at the idle cap |
